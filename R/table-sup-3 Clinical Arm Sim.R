#------------------------------------------------------------------------------#
#------------------------------------------------------------------------------#
# table-sup-3 Clinical Arm Sim.R
#------------------------------------------------------------------------------#
#------------------------------------------------------------------------------#
...
sim_trial <- function(x, arm.size=100) {
  ...
  ARM1 <- PI.helper(srs.obj$DrugModels$Il12, ARM1, 'il12', 100) # drug class fit, PI
  ARM1 <- ARM1 %>% 
    mutate(
      plac.fit = predict(srs.obj$PlaceboModel, newdata=ARM1, re.form=~0), 
      Response = 1 - pnorm(100, # clinical response
                           mean = plac.fit + il12.pi.fit,
                           sd = il12.pi.sd))
  
  ARM2 <- sample_cohort(x, size=arm.size)
  ARM2 <- PI.helper(srs.obj$DrugModels$TNFi, ARM2, 'tnfi', 100)
  ARM2 <- ARM2 %>% 
    mutate(
      plac.fit = predict(srs.obj$PlaceboModel, newdata=ARM2, re.form=~0), 
      Response = 1 - pnorm(100, # clinical response
