...
srs.predict <- function(object, newdata, interval = c('none','confidence','prediction'), 
                        level = 0.95, method = c('analytical','bootstrap'), 
                        model.name = '', nsim = 100, 
                        parallel = c('no','multicore'), ncpus = 1, seed = 1234) 
{
  if(model.name=='') {
    warning("Make sure to specify a model.name.\n")
  }
...
batch.srs.predict <- function(object.list, newdata, 
                              ...
                              nsim = 100, parallel = c('no','multicore'), ncpus = 1, seed = 1234) 
{
  ...
  if (ncores == 1) {
    warning("Only 1 core detected.\n")
    parallel = 'no'
    ncpus = 1
  }
  
  name.list <- names(object.list)
  for(i in 1:length(object.list)) {
    
    if (!inherits(object.list[[i]], "lmerMod")) {
      stop("Error: The provided model is not an lmerMod object. Please provide a valid lmerMod model.")
    }
    
    newdata <- srs.predict(object.list[[i]], newdata=newdata, interval=interval,
                           level=level, method=method, model.name=name.list[[i]], 
